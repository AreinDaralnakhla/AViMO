...
class Custom(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        # Load NLF model output
        nlf_path = osp.join(self.root_path, 'updated-nlf-final-filtered.pkl')
        with open(nlf_path, 'rb') as f:
            nlf_data = pickle.load(f)

        # Extract j2d_smplx keypoints and calculate bounding boxes
        self.nlf_humans = {}
        for frame_idx, humans in zip(frame_idx_list, nlf_data['allFrameHumans']):
            if len(humans) > 0:
                human = humans[0]  # Assume one human per frame
                j2d_smplx = np.array(human['j2d_smplx'], dtype=np.float32)
                self.nlf_humans[frame_idx] = {'j2d_smplx': j2d_smplx}
        ...
        smplx_param_path_list = glob(osp.join(self.root_path, 'smplx_params', '*.json'))
    ...
    def load_id_info(self):
        with open(osp.join(self.root_path, 'smplx_offsets', 'shape_param.json')) as f:
            shape_param = torch.FloatTensor(json.load(f))
        with open(osp.join(self.root_path, 'smplx_offsets', 'face_offset.json')) as f:
            face_offset = torch.FloatTensor(json.load(f))
        with open(osp.join(self.root_path, 'smplx_offsets', 'joint_offset.json')) as f:
            joint_offset = torch.FloatTensor(json.load(f))
        with open(osp.join(self.root_path, 'smplx_offsets', 'locator_offset.json')) as f:
    ...
    def __getitem__(self, idx):
        ...
        # Load mask
        mask = cv2.imread(self.mask_paths[frame_idx])[:, :, 0, None] / 255.0
        mask = self.transform((mask > 0.5).astype(np.float32))

        # Get human data from NLF
        human_data = self.nlf_humans[frame_idx]
        joint_img = human_data['j2d_smplx']  # Shape (127, 2)

        # Create dummy confidence scores (all 1.0 since NLF doesn't provide them)
        joint_conf = np.ones((joint_img.shape[0], 1), dtype=np.float32)

        # Calculate bbox using the original pipeline's `get_bbox` function
        bbox = get_bbox(joint_img, joint_conf[:, 0])  # Uses original extend_ratio=1.2
