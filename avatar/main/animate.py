...
#             # if len(cam_param['R'].shape) == 1 and cam_param['R'].numel() == 4:  # Check if R is a quaternion
#             #     cam_param['R'] = quaternion_to_rotation_matrix(cam_param['R'])
...
# for rendering only the 'render' portion
...
def main():
    ...
    # Load the first original image to get the render shape
    first_frame_path = osp.join(args.motion_path, 'frames', f"{frame_idx_list[0]}.png")
    original_img = cv2.imread(first_frame_path)
    render_shape = original_img.shape[:2]

    # Initialize the video writer for the render section only
    video_out = cv2.VideoWriter('render_only.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (render_shape[1], render_shape[0]))
    ...
    for frame_idx in tqdm(frame_idx_list):
        ...
        # Render only the 'render' portion
        render = (human_render['img'].cpu().numpy().transpose(1, 2, 0)[:, :, ::-1] * 255).copy().astype(np.uint8)

        # Resize the rendered frame to match the original image dimensions (if necessary)
        if render.shape[:2] != original_img.shape[:2]:
            render = cv2.resize(render, (original_img.shape[1], original_img.shape[0]))

        # Write the render frame to the video
        video_out.write(render)

    # Release the video writer
    video_out.release()
    print("Video saved as 'render_only.mp4'")
