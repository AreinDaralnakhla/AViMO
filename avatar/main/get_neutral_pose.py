...
def main():
    ...
    video_path = osp.join(save_path, 'rotation_video.mp4')
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video_writer = cv2.VideoWriter(video_path, fourcc, 30, render_shape)
    ...
    for i in range(view_num):
        ...
        img = human_render['img'].cpu().numpy().transpose(1,2,0)[:,:,::-1]*255
        cv2.imwrite(osp.join(save_path, str(i) + '.png'), img.astype(np.uint8))
